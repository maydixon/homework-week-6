...
```{r}
df<-cbind(KC$log(Brain_Size_Species_Mean), KC$log(MaxLongevity_m))
Long_Pred<- predict(BL, newdata=data.frame(log(Brain_Size_Species_Mean)=KC$log(Brain_Size_Species_Mean)))# predicts values of longevity given our model and all the brain sizes
Brain_Size<-KC$log(Brain_Size_Species_Mean)
Max_Long<-KC$log(MaxLongevity_m)
df<-cbind(Brain_Size, Max_Long, Long_Pred)
names(df)<- c("b_size", "long", "pred_long")
head(df)
ci <- predict(BL, newdata = data.frame(log(Brain_Size_Species_Mean)=KC$log(Brain_Size_Species_Mean)), interval = "confidence",  level = 0.90)  # conf for all 

pi <- predict(BL, newdata = data.frame(log(Brain_Size_Species_Mean)=KC$log(Brain_Size_Species_Mean)), interval = "prediction",  level = 0.90)  # predict all 
